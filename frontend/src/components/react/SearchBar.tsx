...
export default function SearchBar({ type, elements, setElements }: Parametres) {
  ...
  return (
    <div className="w-full xl:w-72 relative pb-[3px] focus-within:gradient-border">
      <svg
      ...
      <input
        ...
        placeholder={`Buscar ${
          type === "paises" ? "país"
        : 
          type === "provincias" ? "provincia, país"
        : 
          type === "localidades" ? "localidad, provincia, país"
        : 
          type === "solicitudes" ? "email, país, provincia, localidad"
        : 
          type === "usuarios" ? "email, nombre, apellido"
        :
          type === "usuario-solicitudes" ? "país, provincia, localidad"
        : 
          "email, país, provincia, localidad"
        }...`}
      />
    </div>
  );
