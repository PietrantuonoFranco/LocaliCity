...
export default function Paises() {
  ...
  const getSortedData = () => {
    if (!sortField || !paises) return paises

    return [...paises].sort((a: Pais, b: Pais) => {
      let aValue = ""
      let bValue = ""
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setPaises(prevPaises => 
        prevPaises ? prevPaises.filter(pais => pais.id !== id) : []
      );
    } catch (error) {
          ...
          <div className="h-full flex flex-col">
            <div className="w-full flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <h2 className="text-3xl font-bold text-gray-800">Paises</h2>
                      ...
                      <tr key={pais.id} className="hover:bg-white/35 group">
                        ...
                        <td className="px-4 py-2 group-hover:rounded-r-2xl">
                          <div className="flex space-x-2 items-center">
                            {/* Boton de editar */}
                            <a
                              href={`/paises/editar/${pais.id}`} target="blank" rel="noopener noreferrer"
